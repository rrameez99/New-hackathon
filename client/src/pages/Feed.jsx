...
export default function Feed({ games, joinedIds, onJoin, onLeave, loading, onGameClick }) {
    ...
    <div className="animate-fade-in">
      ...
      <div className="pt-2 pb-5">
        ...
        <p className="text-gray-600 text-sm mt-1">
          {loading ? "Loading games..." : `${filtered.length} ${filtered.length === 1 ? "game" : "games"} happening near you`}
        </p>
      </div>
        ...
        {SPORTS.map((sport) => (
          <button
            ...
            className={`shrink-0 px-4 py-2 rounded-full text-sm font-medium border transition-all whitespace-nowrap ${
              filter === sport.id ? "" : "border-brand-border bg-brand-card text-gray-500"
            }`}
            style={
              filter === sport.id
      ...
      <div className="flex flex-col gap-3 pb-6">
        {loading ? (
          <div className="text-center py-16">
            ...
            <p className="font-display text-lg font-bold text-gray-600">Loading games...</p>
          </div>
        ) : filtered.length === 0 ? (
          <div className="text-center py-16">
